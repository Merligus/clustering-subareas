...
def _smacof_single(dissimilarities, metric=True, n_components=2, init=None, 
                    weights=None, max_iter=300, verbose=0, eps=1e-3, random_state=None):        
    ...
    opcao_n = 1
    if opcao[opcao_n] == 'um':
        weights = np.ones_like(dissimilarities)
    elif opcao[opcao_n] == 'um_ou_zero':
        weights = np.where(dissimilarities == np.max(dissimilarities), 0., 1.)
    elif opcao[opcao_n] == '1/d':
        dissimilarities = np.where(dissimilarities == 0., np.inf, dissimilarities)
        d_min = np.min(dissimilarities)
        weights = (1. / dissimilarities) / (1. / d_min)
        dissimilarities = np.where(dissimilarities == np.inf, 0, dissimilarities)
    elif opcao[opcao_n] == '1/d2':
        dissimilarities = np.where(dissimilarities == 0., np.inf, dissimilarities)
        d_min = np.min(dissimilarities)
        weights = (1. / dissimilarities ** 2) / (1. / d_min ** 2)
        dissimilarities = np.where(dissimilarities == np.inf, 0, dissimilarities)
